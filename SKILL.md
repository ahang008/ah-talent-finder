...
3. **无存档**：`init` → 读 `steps/Step1-轻量画像收集.md` → 开始节点1
...
| 文件 | 用途 | 何时读取 |
|------|------|---------|
| steps/Step1-轻量画像收集.md | 8节点对话流程 + 画像模板 + 决策点 | 每次必读 |
| steps/Step2-深度版画像.md | 过去+内在探索 + 内心原密码报告 | 用户选深度版时读 |
| steps/Step3-职业方向与验证方案.md | 5方向推荐 + 匹配度 + 2周验证 | 画像完成后读 |
| steps/Step4-命理交叉验证.md | 八字排盘 + 古典命理交叉验证 | 用户选命理验证时读 |
| reference/天赋理论框架库.md | 9个心理学框架 | 分析阶段后台参考 |
| reference/追问规则库.md | 各节点提取字段速查 | 信息收集时参考 |
